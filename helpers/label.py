...
class Label(object):
    def __init__(self, pkg, picking=None, from_address=None, to_address=None, customs=None, config=None, test=None):
        ...
        if config == None:
            config = settings.FEDEX_CONFIG

        # Are we running in test mode? If so, we'll
        # need to use FedEx's sandbox servers.
        if picking and picking.logis_company and test == None:
            config.use_test_server = picking.logis_company.test_mode

        self.api = fedex_wrapper.FedEx(config)
    ...
    def generate(self, service, image_format="EPL2"):
        if service == None and self.picking:
            service = self.picking.fedex_service_type
        ...
        raise Exception(label.get("error", "Could not generate FedEx label!"))